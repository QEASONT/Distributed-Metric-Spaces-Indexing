...
double ConfigTool::GetConfigFloat(const char* config_key, const char *prefix)
{
	double value=0;
	...
	{
		const char *val=the_map[cfg_key].c_str();
		if(IfNumChar(val[0])) value=(double)atof(the_map[cfg_key].c_str());
...
int ConfigTool::GetConfigFloatArray(const char* config_key, double *data_array, const char *prefix)
{
	char *str=new char[line_max];
	...
	{	   
		*q='\0';
		data_array[count++]=(double)atof(p);
		p=q+1;
	}
	data_array[count++]=(double)atof(p);
