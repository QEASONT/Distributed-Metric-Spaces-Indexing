...
void lshknn(double c, int k, Hash& myslsh, Preprocess& prep, double beta, std::string& datasetName, std::string& data_fold);
void expe_k(double c, Hash& myslsh, Preprocess& prep, double beta, std::string& datasetName, std::string& data_fold);
...
int main(int argc, char const* argv[])
{
	...
	else//only for debug, not advised for user
	{
		...
		exit(-1);*/
		unsigned dims[2] = { 100000,200 };
		datasetName = "d" + std::to_string(dims[1]) + "s" + std::to_string(dims[0]);

		const std::string datas[] = { "audio","mnist","cifar","NUS","Trevi","gist","deep1m","glove" };
		//datasetName = datas[0];
		//datasetName = "d200";
...
#else
	std::string data_fold = "D:\\Code\\Metric_Space_Indexing\\DB-LSH\\dataset\\", index_fold = "";
#endif

	Preprocess prep(data_fold + datasetName + ".data", data_fold + datasetName + ".bench");
	...
	if (beta > 0) {
		...
		std::vector<double> Betas = { 0.02,0.04,0.06,0.08,0.10,0.12,0.14,0.16,0.18,0.2 };
...
void range_query(double c, int k, Hash& myslsh, Preprocess& prep, double beta, std::string& datasetName, std::string& data_fold) {
	lsh::timer timer;
	...
	double mean_time = (double)perform.time_total / perform.num;
	std::cout << "AVG QUERY TIME:    " << mean_time * 1000 << "ms." << std::endl << std::endl;
	//std::cout << "SORT TIME:         " << ((double)perform.time_sift) / (perform.num) << std::endl;
	//std::cout << "AVG QUERY TIME:    " << (double)perform.time_verify / perform.num * 1000 << "ms." << std::endl << std::endl;
	std::cout << "AVG RECALL:        " << ((double)perform.NN_num) / (perform.num * k) << std::endl;
	std::cout << "AVG RATIO:         " << ((double)perform.ratio) / (perform.res_num) << std::endl;
	std::cout << "AVG COST:          " << ((double)perform.cost) / ((double)perform.num * prep.data.N) << std::endl;
	std::cout << "AVG ROUNDS:        " << ((double)perform.rounds) / (perform.num) << std::endl;
	//std::cout << "AVG ACCESSES:      " << ((double)perform.num_access_in_RTree) / (perform.num) << std::endl;
	std::cout << "\nQUERY FINISH... \n\n\n";

	time_t now = std::time(0);
	time_t zero_point = 1635153971;//Let me set the time at 2021.10.25. 17:27 as the zero point
	double date = ((double)(now - zero_point)) / 86400;
	...
	if (os) {
		...
		os << dataset << ',' << c << ',' << k << ',' << myslsh.L << ',' << myslsh.K << ',' << myslsh.R_min << ','
			<< ((double)perform.ratio) / (perform.res_num) << ','
			<< ((double)perform.NN_num) / (perform.num * k) << ','
			<< mean_time * 1000 << ','
			<< ((double)perform.cost) / (perform.num * prep.data.N) << ','
			<< date << ','
			<< std::endl;
		os.close();
	}
