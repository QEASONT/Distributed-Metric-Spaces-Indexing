...
Preprocess::Preprocess(const std::string& path, const std::string& ben_file_, double beta_)
{
...
void Preprocess::load_data(const std::string& path)
{
	...
	assert(header[0] == sizeof(double));
	data.N = header[1];
	data.dim = header[2];

	data.val = new double* [data.N];
	for (int i = 0; i < data.N; ++i) {
		data.val[i] = new double[data.dim];
		//in.seekg(sizeof(double), std::ios::cur);
		in.read((char*)data.val[i], sizeof(double) * header[2]);
	}
...
//		assert(header[1] != 0);
//		double* array_ = new double[header[2] * header[1]];
//		is.read((char*)&array_[0], sizeof(double) * header[2] * header[1]);
//		data.N = header[1];
//		//data.N = 100;
//		data.dim = header[2];
...
//			out.write((char*)data.val[i], sizeof(double) * header[2]);
//		}
//
//		out.close();
//
...
//			outq.write((char*)data.query[i], sizeof(double) * header[2]);
//		}
//
//		outq.close();
//
...
void Preprocess::ben_make()
{
	...
	benchmark.dist = new double* [benchmark.N];
	for (unsigned j = 0; j < benchmark.N; j++) {
		benchmark.indice[j] = new int[benchmark.num];
		benchmark.dist[j] = new double[benchmark.num];
	}
...
void Preprocess::ben_save()
{
	...
	for (unsigned j = 0; j < benchmark.N; j++) {
		out.write((char*)&benchmark.dist[j][0], sizeof(double) * benchmark.num);
	}
...
void Preprocess::ben_load()
{
	...
	for (unsigned j = 0; j < benchmark.N; j++) {
		benchmark.dist[j] = new double[benchmark.num];
		in.read((char*)&benchmark.dist[j][0], sizeof(double) * benchmark.num);
	}
	in.close();
}
...
Parameter::Parameter(Preprocess& prep, unsigned L_, unsigned K_, double rmin_)
{
	N = prep.data.N;
	dim = prep.data.dim;
	L = L_;
