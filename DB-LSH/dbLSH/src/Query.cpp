...
Query::Query(unsigned id, double c_, unsigned k_, Hash& hash, Preprocess& prep, double beta_)
{
	flag = id;
	c = c_;
	k = k_;
...
void Query::sift(Hash& hash, Preprocess& prep)
{
	...
	if (hash.N < 70000) {
		...
		//200-1001
		t = 200 + 7 * ((double)hash.N) / 10000;
		t = 1000;
	}
	else if (hash.N >= 500000 && hash.N < 2000000) {
		//501-1000
		t = 501.0+(1000.0-501.0)/150.0* ((double)hash.N) / 10000;
		t = 2000;
	}
	else if (hash.N >= 2000000 && hash.N < 2000000000) {
		//1000-20000
		t = 1000.0 + 19000.0 / 200000.0 * ((double)hash.N) / 10000;
		t = 20000;
	}
	...
	while (! visits->termination && rounds<=30) {
		...
		for (int i = 0; i < hash.L; ++i) {
			int sta = i * hash.K;
			visits->q_mbr = new double[2 * visits->low_dim];
...
void Performance::update(Query &query, Preprocess &prep)
{
	...
	{
		double rate = query.res[j].dist / prep.benchmark.dist[query.flag][j];
