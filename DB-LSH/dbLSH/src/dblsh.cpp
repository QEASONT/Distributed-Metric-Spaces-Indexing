...
void Hash::SetHash()
{
	hashpar.rndAs1 = new double* [S];
	hashpar.rndAs2 = new double* [S];


	for (unsigned i = 0; i < S; i++) {
		hashpar.rndAs1[i] = new double[dim];
		hashpar.rndAs2[i] = new double[1];
	}

	std::mt19937 rng(unsigned(0));
	//std::mt19937 rng(unsigned(std::time(0)));
	std::normal_distribution<double> nd;//nd is a norm random variable genarator��mu=0��sigma=1
...
void Hash::GetHash(Preprocess& prep)
{
	...
	hs.resize(L);
	for (int i = 0; i < L; ++i) {
		hs[i] = new (TreeDataP<double>*[N]);
	}
	showMemoryInfo();
	for (int i = 0; i < L; ++i) {
		for (int j = 0; j < N; ++j) {
			hs[i][j] = new TreeDataP<double>(K);
			hs[i][j]->id = j;
			for (int k = 0; k < K; ++k) {
				//hs[i][j]->data[k] = 0;
				hs[i][j]->data[k] = cal_inner_product(prep.data.val[j], hashpar.rndAs1[i * K + k], dim);
				//cout << hs[i][j]->data[k] << endl;
			}
		}
	}
	showMemoryInfo();
...
void Hash::GetTables(Preprocess& prep)
{
	...
	myIndexes = new RStarTree<TreeDataP, double>*[L];
	...
	if (N > 1) {
		...
		for (int i = 0; i < L; ++i) {
			...
			myIndexes[i] = new RStarTree<TreeDataP, double>(file.c_str(), K, page_len, ".",
