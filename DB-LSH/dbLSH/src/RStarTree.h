...
inline 
bool Is_Intersect(double* mbr, double* data, const int& dim)
{
	//return true;
	//bool flag_intersect;
	if (dim == 10) {
...
inline
bool Is_enclosing(double* qmbr, double* nmbr, const int& dim)
{
	//return true;
	//bool flag_intersect;
	if (dim == 10) {
...
public:
	...
	Visitor(int N, int K_, int dim_, int k_,int MaxC, double** mydata, double* query, double R_min) {
		flag = new int[N];
		for (int i = 0; i < N; ++i) {
			flag[i] = true;
		}
...
//T is better to be int, double or double, default is double
template <typename T = double>
class TreeDataP
{
public:
...
template <template <typename TP> class Data = TreeDataP, typename T = double> class RStarTree;
template <template <typename TP> class Data = TreeDataP, typename T = double> class RSTNode;
template <template <typename TP> class Data = TreeDataP, typename T = double> class RSTNonLeafNode;
template <template <typename TP> class Data = TreeDataP, typename T = double> class RSTLeafNode;
	...
	for (int j = 0; j < this->entry_num; ++j) {
		...
		if (
			...
			if (visit->flag[data_id]) {
				...
				visit->res[visit->count].dist = calc_l2_dist<double>(visit->data_dim, visit->original_data[data_id], visit->q_point);
